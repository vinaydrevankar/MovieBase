...
export const serviceGetMoviesList = (searchText="gangster") => (dispatch:AppDispatch) => Promise.resolve().then(async () => {
    try {
        dispatch({ type: REDUX_CONSTANTS.SERVICE_GET_MOVIE_LIST_LOADING });
        const response:any = await getDataFromService({ url: HOME_API_URL+searchText })
